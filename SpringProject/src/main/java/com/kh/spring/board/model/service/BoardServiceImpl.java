...
public class BoardServiceImpl implements BoardService{
	 ...
	 * 
	 * 방법 2) @Transactional 어노테이션 활용하여 선언적 트랜젝션 처리
	...
	public int insertBoard(Board b, List<MultipartFile> list, String webPath, String serverFolderPath) throws Exception {
		...
		if(boardNo > 0 && list != null) {
			...
			// list에서 담겨있는 파일 정보 중 실제로 업로드된 파일만 분류하기
			for(int i = 0; i < list.size(); i++) {
				if(list.get(i).getSize() > 0) { // i번째 요소에 업로드된 이미지가 존재하는지 체크
					...
					// BoardImg 객체를 생성해서 필드에 값을 세팅한 후 boardImgList에 추가할 예정
			...
			// 분류작업 완료 후 boardImgList가 비어있는가 ? 등록한 이미지가 없음
			//							비어있지않다 ? 등록한 이미지가 있음
