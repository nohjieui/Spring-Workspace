...
@Aspect		// 공통관심사가 작성된 클래스임을 명시
			...
			//   Aspect어노테이션이 붙은 클래스에는 실행할 코드(advice)와 @PoinCut이 정의되어있어야한다.
			//   advice(끼어들어서 실제로 수행할 메서드, 코드)
			//   @PoinCut(advice가 끼어들어서 수행될 클래스, 메서드 위치들을 정의)가 작성되어 있어야한다.
public class Test {
	private Logger logger = LoggerFactory.getLogger(Test.class);
	
	// joinPoint : 클라이언트가 호출하는 모~든 비지니스 메서드, advice가 적용될 수 있는 예비 후보
	// 			   ex) 클래스의 인스턴스 생성지점. 메소드 초출시점, 예외 발생들 전부
	 ...
	 * 
	 * @Before : PoinCut에 지정된 메소드가 수행되기 "전"에 advice 수행을 명시하는 어노테이션
	 * 
